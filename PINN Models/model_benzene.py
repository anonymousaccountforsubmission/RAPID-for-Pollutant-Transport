...
SCENARIO_PATH = DATA_BASE / "benzenedata"
COORD_FRAME_PATH = SCENARIO_PATH / "coordinate_frame.json"
NORM_META_PATH = SCENARIO_PATH / "normalization_metadata.json"

MODEL_BEST = SAVE_PATH / "houston_pinn_curriculum_lowwind_physics_best.pth"
MODEL_FINAL = SAVE_PATH / "houston_pinn_curriculum_lowwind_physics_final.pth"
METRICS_OUT = SAVE_PATH / "houston_pinn_curriculum_lowwind_physics_metrics.csv"
TRAIN_LOG_OUT = SAVE_PATH / "houston_pinn_curriculum_lowwind_physics_training_log.csv"
...
LOW_WIND_THRESHOLD = 1.0       # m/s; dataset u/v are already floored near 0.707, so 1.0 captures low-wind structural-zero rows
...
SIGNAL_UNDERPRED_PENALTY = 3.0   # multiply squared log error when pred < true
HIGH_LABEL_THRESHOLD     = 5.0   # ug/m3, defines a "high" AERMOD row (matches metric)
HIGH_CAPTURE_TARGET      = 5.0   # ug/m3, pull high-row preds up toward this (well above the 2.5 bar)
HIGH_CAPTURE_WEIGHT      = 0.5   # weight of the explicit high-value recall hinge
STRUCTURAL_PHYSICS_WEIGHT = 1.0
STRUCTURAL_PHYSICS_N_POINTS = 256
STRUCTURAL_PHYSICS_GLOBAL = True
PHYSICS_LENGTH_SCALE = 2000.0       # m, characteristic source/spread length for dimensionless residual scaling
PHYSICS_CONC_FLOOR = 1e-4           # ug/m3, prevents residual scaling from vanishing near zero

# Adaptive physics balancing.

PHYS_ADAPTIVE_BALANCE = True
SIGNAL_PHYS_TARGET_RATIO = 0.25      # weighted signal physics ≈ 25% of data loss
STRUCTURAL_PHYS_TARGET_RATIO = 0.5   # structural rows carry no data label, so allow more
PHYS_EFF_WEIGHT_MAX = 1.0            # safety cap on the adaptive weight

if not DRIVE_BASE.exists():
    raise FileNotFoundError(f"Drive folder not found: {DRIVE_BASE}")

# Allow the regenerated folder to use a slightly different scenario folder name.
...
# coordinate_frame.json is optional for this dataset. When it is missing the
# x/y domain bounds are derived from the loaded collocation data instead.
...
class ParametricADEPINN(nn.Module):
    """
    Parametric 3D advection-diffusion PINN surrogate for AERMOD concentration labels.
    ...
    Inputs:
        ...
        kappa   : turbulent diffusivity (m²/s)
        Q       : emission rate (g/s)

    Output:
        phi     : AERMOD-labeled concentration contribution (µg/m³)
    """
...
def _load_npz_array(npz_file):
    ...
    with np.load(npz_file) as npz:
        if 'data' in npz.files:
            ...
            # Fall back to the first stored array if the key is not 'data'.
            arr = npz[npz.files[0]]
    return arr.astype(np.float32)


def load_houston_data(scenario_path):
    """
    Load AERMOD-only Houston scenario npz files (3D, sharded).
    ...
    for folder in sorted(Path(scenario_path).iterdir()):
        ...
        # Backwards-compatible fallback to a single unsharded file.
        if not shard_files:
...
def compute_pde_residual_fast(model, x, y, z, t, cx, cy, h0, u, v, d, kappa, Q, dev):
    """
    Steady 3D advection-diffusion residual for the PINN regularizer.

    Because Houston rows use fixed t=3600, phi_t is set to zero, and because the
    dataset carries no vertical wind, vertical advection (w*phi_z) is omitted.
    Diffusion acts in all three dimensions. The model is still trained primarily
    as a supervised AERMOD surrogate.
    """
    ...
    residual = phi_t + u_phys*phi_x + v_phys*phi_y - kappa*(phi_xx + phi_yy + phi_zz) - source

    # Dimensionless residual scaling. The previous scale, abs(phi)+abs(source),
    
    speed = torch.sqrt(u_phys**2 + v_phys**2).clamp(min=0.5)
    c_ref = phi.abs().clamp(min=PHYSICS_CONC_FLOOR)
    L = PHYSICS_LENGTH_SCALE
    scale_n = (speed * c_ref / L + kappa * c_ref / (L * L) + source.abs()).clamp(min=1e-12)
    return (residual / scale_n).clamp(-10, 10)
...
def compute_regime_aware_data_loss(phi_pred, phi_true, regime):
    ...
    n_total = max(int(regime.numel()), 1)
    ...
    if mask_signal.any():
        ...
        # Asymmetric error. Under-prediction (log_diff < 0) is exactly what
        # drives high-label capture below 100%, so penalize it more heavily than
        # over-prediction. SIGNAL_UNDERPRED_PENALTY == 1 recovers symmetric MSE.
        asym_w = torch.where(
            log_diff < 0,
        ...
        # Magnitude-weighted log loss: large AERMOD values carry more weight so
        # the network stops compressing the high-concentration tail. With
        # SIGNAL_HIGH_VALUE_WEIGHT_POWER == 0 this reduces to a plain mean.
        ...
        # Explicit high-value recall hinge. This directly optimizes the capture
        # metric: for AERMOD>5 rows it penalizes only the shortfall below
        # log10(HIGH_CAPTURE_TARGET). Because it is one-sided it never pushes
        # high rows down and never touches the low-concentration field, so it
        # raises capture without inflating low-side bias.
        high_mask = true1 > HIGH_LABEL_THRESHOLD
        if high_mask.any() and HIGH_CAPTURE_WEIGHT > 0:
            log_target = float(np.log10(HIGH_CAPTURE_TARGET))
    ...
    if mask_phys_zero.any():
        ...
        # Weak upper bound. This does not force exact zero, but discourages
        # large spurious concentrations where non-low-wind AERMOD is zero.
...
def compute_surrogate_loss(phi_pred, phi_true):
    """Baseline log-space loss retained for ablation/debugging only."""
    log_diff = (
        torch.log10(phi_pred.clamp(min=1e-15)) -
        torch.log10(phi_true.clamp(min=1e-15))
    ).clamp(-5, 5)
    loss = torch.mean(log_diff**2)
    return loss, {'log_mse': loss.item()}

# =============================================================================
# BENCHMARKING METRICS — PINN VS AERMOD LABELS
# =============================================================================

def compute_benchmark_metrics(pred_np, true_np, label):
    """Agreement metrics for PINN predictions against held-out AERMOD labels."""
    ...
    high_true = true_s > 5.0
    high_capture = float((pred_s[high_true] > 2.5).mean()) if high_true.sum() > 0 else 0.0
    ...
    print(f"    High-label capture={high_capture:.3f}  (AERMOD>5 & PINN>2.5 µg/m³)")
...
def evaluate_full_benchmark(model, tensors, set_name, max_samples=100000):
    ...
    if high_mask.sum() > 20:
        rows.append(compute_benchmark_metrics(
            ...
            f"{set_name} — High AERMOD labels (>5)"))
    ...
    if physical_zero_mask.sum() > 0:
        ...
        print(f"    PINN > 0.1 µg/m³:        {(pred_np[physical_zero_mask] > 0.1).mean()*100:.1f}%")
        print(f"    PINN > 0.5 µg/m³:        {(pred_np[physical_zero_mask] > 0.5).mean()*100:.1f}%")
    ...
    if structural_zero_mask.sum() > 0:
        ...
        print(f"    PINN > 0.1 µg/m³:        {(pred_np[structural_zero_mask] > 0.1).mean()*100:.1f}%")
        print(f"    PINN > 0.5 µg/m³:        {(pred_np[structural_zero_mask] > 0.5).mean()*100:.1f}%")
    ...
    if zero_mask.sum() > 0:
        ...
        print(f"    PINN > 0.1 µg/m³ on zero-label rows: {(pred_np[zero_mask] > 0.1).mean()*100:.1f}%")
...
print("\n" + "="*60)
print("HOUSTON SHIP CHANNEL BENZENE PINN — REGIME-AWARE AERMOD-DERIVED MODEL")
print("="*60)

print("\n1. Loading AERMOD-only Houston data...")
houston_data = load_houston_data(SCENARIO_PATH)
...
print(f"  kappa range:  [{model.kappa_min.item():.1f}, {model.kappa_max.item():.1f}] m²/s")
...
# This prevents the PDE residual from shaping a randomly initialized network.
...
print(f"  Physical-zero upper bound: {PHYSICAL_ZERO_UPPER_BOUND} µg/m³")
...
print("\n4. Training regime-aware AERMOD-derived model...")
print("-" * 60)
...
for epoch in range(num_epochs):
    ...
    for b in range(N_BATCHES):
        ...
        if struct_ramp > 0 and STRUCTURAL_PHYSICS_GLOBAL and len(STRUCTURAL_TRAIN_IDX) > 0:
            ...
            res_struct = compute_pde_residual_fast(
                model,
        ...
        if b % PHYSICS_EVERY_N == 0 and sig_ramp > 0 and len(SIGNAL_TRAIN_IDX) > 0:
            ...
            res_signal = compute_pde_residual_fast(
                model,
    ...
    if not np.isnan(avg_data) and avg_data < best_loss:
        ...
        torch.save({
            ...
            'training_mode': 'curriculum_lowwind_physics_aermod_derived',
        }, MODEL_BEST)
    else:
...
print("\n6. Running PINN-vs-AERMOD benchmark suite...")
...
if phys_nonzero:
    ...
    ax.semilogy(pe, pv, 'r-', lw=1.5, alpha=0.7, label='Physics Loss')
...
plt.tight_layout()
fig_path = SAVE_PATH / 'training_progress_lowwind_physics.png'
plt.savefig(fig_path, dpi=150, bbox_inches='tight')
plt.show()
print(f"✓ Training plot saved: {fig_path}")

# =============================================================================
# HOUSTON DOMAIN CONCENTRATION FIELD
# =============================================================================

def visualize_houston_field(model, u_eff, v_eff, kappa_val, source_params,
    ...
    cbar.set_label('log₁₀(AERMOD-surrogate benzene µg/m³)')
    ...
    ax.set_title(f"AERMOD-Surrogate Benzene Field | z={z_eval:.0f}m u={u_eff:.1f} v={v_eff:.1f} κ={kappa_val:.0f} m²/s")
    ax.set_aspect('equal')
    plt.tight_layout()
...
if met_path.exists() and src_path.exists():
    ...
    print(f"\n  Generating field plot: z={z_field:.0f} u={ex_u:.2f} v={ex_v:.2f} κ={ex_k:.0f}")
    visualize_houston_field(
        model, ex_u, ex_v, ex_k, source_params_ex,
        z_eval=z_field,
        save_path=SAVE_PATH / 'houston_concentration_field_lowwind_physics.png'
    )
else:
...
def visualize_plume_3d(model, scenario, n_xy=48, n_z=28, half_width_m=15000.0,
    ...
    """
    Render a single 3D plume scenario as a volumetric scatter.
    ...
    ax.set_title(
        f"3D Plume Scenario | h0={h0:.0f}m  Q={Q:.2e} g/s\n"
        f"u={u:.2f} v={v:.2f} (|U|={spd:.2f} m/s)  κ={k:.0f} m²/s"
    )
...
# Using actual data guarantees this runs even without the optional parquet files.
...
print(f"  Source @ ({plume_scenario['cx_local']/1000:.1f}, "
      ...
      f"κ={plume_scenario['kappa']:.0f}")

visualize_plume_3d(
    model, plume_scenario,
    save_path=SAVE_PATH / 'houston_plume_3d_lowwind_physics.png'
)
...
ax.set_xlabel('AERMOD label log₁₀(µg/m³)')
ax.set_ylabel('PINN prediction log₁₀(µg/m³)')
...
if valid_mask.sum() > 0:
    ...
    ax.axvline(mu - sigma, color='orange', lw=1.5, ls='--', label=f'±1σ: {sigma:.3f}')
...
ax.set_xlabel('Log₁₀ Residual (PINN − AERMOD)')
ax.set_ylabel('Density')
ax.set_title('Residual Distribution — Test Regime 1 Signal Rows')
ax.grid(True, alpha=0.3)

plt.tight_layout()
parity_path = SAVE_PATH / 'parity_plot_lowwind_physics.png'
...
print("\n" + "="*60)
print("CURRICULUM LOW-WIND PHYSICS PINN TRAINING & BENCHMARKING COMPLETE")
print("="*60)
...
print(f"  3D plume plot:       {SAVE_PATH / 'houston_plume_3d_lowwind_physics.png'}")
print(f"  Parity plot:         {parity_path}")

print(f"\n  Training summary:")
print(f"    Mode:              Curriculum low-wind physics AERMOD-derived PINN")
